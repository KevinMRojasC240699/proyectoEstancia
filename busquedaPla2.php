...
if ($result && mysqli_num_rows($result) > 0) {
    ...
    if ($result_nutriologo && mysqli_num_rows($result_nutriologo) > 0) {
        ...
        // Obtener los platillos asociados a los pacientes del nutriólogo con búsqueda
        $sql_platillos = "SELECT p.id_pla, p.nombre_platillo, p.descripcion, p.cantidad
                      ...
                      AND p.nombre_platillo LIKE '%$search%'";
        ...
        if ($result_platillos && mysqli_num_rows($result_platillos) > 0) {
            ...
            echo "<p>No se encontraron platillos para este nutriólogo.</p>";
        }
    } else {
        echo "<p>Error al obtener el ID del nutriólogo.</p>";
    }
} else {
